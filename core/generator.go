...
func Generators(req libs.Request, sign libs.Signature) []libs.Request {
	...
	for _, payload := range realPayloads {
		...
		// target := req.Target
		req.Target["payload"] = payload
		req.Detections = ResolveDetection(req.Detections, req.Target)
		req.Generators = ResolveDetection(req.Generators, req.Target)
		for _, genString := range req.Generators {
			...
			injectedReqs := RunGenerator(req, genString)
			if len(injectedReqs) <= 0 {
				utils.ErrorF("No request generated by: %v", genString)
				continue
			}
			for _, injectedReq := range injectedReqs {
				...
				injectedReq.Detections = AltResolveDetection(req.Detections, injectedReq.Target)
